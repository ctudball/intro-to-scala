...
object ListExercises {
  ...
  /**
    ...
    * Hint: Use .map
    **/
  def addNumToEach(num: Int, nums: List[Int]): List[Int] = nums.map(a => a + num)
  ...
  /**
    ...
    */
  def filterEven(nums: List[Int]): List[Int] = nums.filter( a => a % 2 == 0)
  ...
  /**
    ...
    * Hint: Use .foldLeft
    */
  def product(nums: List[Int]): Int = nums.foldLeft(1) {(acc,b) => acc * b}
  ...
  def min(nums: List[Int]): Int = nums match {
    ...
    case head :: tail => tail.foldLeft(head) {(acc,b) => if (acc > b) b else acc }
  }
  ...
  def youngestPerson(persons: List[Person]): Person = persons match {
    ...
    case head :: tail => tail.foldLeft(head) {(acc,b) => if (acc.age > b.age) b else acc }
  }
  ...
  /**
    ...
    *
    */
  def showEveryNthPerson(n: Int, persons: List[Person]): List[String] = persons.zipWithIndex
                                                                               .filter(pair => (pair._2+1) % n == 0)
                                                                               .map(x => s"${x._1.name} is ${x._1.age} years old")

...
object OptionExercises1 {
  ...
  def safeMean(nums: List[Int]): Option[Double] = nums match {
    case Nil => None
    case _ => Some(nums.sum / nums.length.toDouble)
  }
  ...
  /**
    ...
    **/
  def mkPerson(name: String, age: Int): Option[Person] = if (age < 0) None else if (name.isEmpty) None else Some(Person(name, age))
